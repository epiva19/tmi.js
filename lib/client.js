...
client.prototype.connect = function connect() {
    return new Promise((resolve, reject) => {
        ...
        this.once("_promiseConnect", (err) => {
            ...
            else { reject(err); this._openConnection(); }
        });
    });
};
...
client.prototype._openConnection = function _openConnection() {
    if(this.opts.useProxy === true) {
        ...
        let x = url.parse('http://' + this.opts.proxies);
        console.log(`Using proxy: ${x.auth}...`)

        let y = new HttpsProxyAgent(x);
        if(this.opts.proxies) this.ws = new ws(`${this.secure ? "wss" : "ws"}://${this.server}:${this.port}/`, { agent: y });
...
client.prototype._onOpen = function _onOpen() {
    ...
    let haha = this.api;
    let hehe = this.clientId;

    /*haha({
        url: `/users/${this.opts.identity.username}/follows/channels/niiko_uk_to_31`,
        headers: {
            "Authorization": `OAuth ${_.password(_.get(this.opts.identity.password, "")).replace("oauth:", "")}`//,
            //"Client-Id": this.opts.identity.clientId
        },
        method: "PUT"
    }, (err, res, body) => {
        if (!err) {
    ...
    /*haha({
        url: `/user?login=${this.opts.channels[0]}`,
        headers: {
            "Authorization": `OAuth u6xetzl86mhzhg1axlpcpdprr9up6g`,
            "Client-Id": '6dzd3obdn59z0agrzsuxomm5gwboeq'
        },
        method: "GET"
    }, (er, re, bo) => {
        if (er) return;
        ...
        var Follower;
        haha({
            url: `/user?login=${this.opts.identity.username}`,
            headers: {
                "Authorization": `OAuth u6xetzl86mhzhg1axlpcpdprr9up6g`,
                "Client-Id": '6dzd3obdn59z0agrzsuxomm5gwboeq'
            },
